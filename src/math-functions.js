...
export function sumAndMultiplyThreeNumbers(a, b, c) { //eslint-disable-line
    ...
    const firstMulti = multiply(a, b)[0];
    const multiTotal = multiply(firstMulti, c)[0];
    const firstString = `${a} and ${b} and ${c} sum to ${totalSum}.`;
    const secondString = `The product of ${a} and ${b} and ${c} is ${multiTotal}.`;

    return [totalSum, multiTotal, firstString, secondString];
}
...
export function sumArrayWithThreeNumbers(sumArr) {
    const firstSumOfArray = sum(sumArr[0], sumArr[1])[0];
    const totalArray = sum(firstSumOfArray, sumArr[2])[0];

    return [totalArray, `${sumArr[0]},${sumArr[1]},${sumArr[2]} was passed in as an array of numbers, and ${totalArray} is their sum.`];
}
...
export function multiplyArrayWithThreeNumbers(multArr) { //eslint-disable-line
    const firstProductOfArray = multiply(multArr[0], multArr[1])[0];
    const totalProArray = multiply(firstProductOfArray, multArr[2])[0];

    return [totalProArray, `The numbers ${multArr[0]},${multArr[1]},${multArr[2]} have a product of ${totalProArray}.`];
}
...
export function multiplyAnyArray(dynamicArray) { //eslint-disable-line
    const firstTwoNums = multiply(dynamicArray[0], dynamicArray[1])[0];
    const thirdAndFourthNums = multiply(dynamicArray[2], dynamicArray[3])[0];
    const fourNumsTotal = multiply(thirdAndFourthNums, firstTwoNums)[0];
    const finalNum = multiply(fourNumsTotal, dynamicArray[4])[0];

    return [finalNum, `The numbers ${dynamicArray[0]},${dynamicArray[1]},${dynamicArray[2]},${dynamicArray[3]},${dynamicArray[4]} have a product of ${finalNum}.`];
}
